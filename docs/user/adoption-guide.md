...
> **Exception:** if the reason you're adopting Activate at all is to prove impact to leadership — not just to improve day-to-day workflow — install the [Activate Metrics module](../../README.md#optional-activate-metrics-module) (`baseline-capture`, `rollout-monitor`, `impact-assessment` skills plus the `activate-metrics-facilitator` agent) from day one, before your baseline conventions are otherwise stable. Measurement infrastructure only works if it's in place *before* rollout starts; waiting until things feel settled defeats the purpose. This is a deliberate, named exception — call it out to the team so it isn't confused with the general "wait on skills/agents" guidance above.
...
Treat your Activate files like code: review them in retrospectives, update instruction files when patterns change, retire prompts that aren't being used. Track which prompts and skills get invoked most and invest there first. If you ran the Activate Metrics module, the week-6 `impact-assessment` report and quality-gate results are the concrete evidence for this stage — use them to decide what to promote, tighten, or retire rather than going on impression alone.
